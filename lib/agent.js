...
只返回 JSON，不要其他内容。`;
    ...
    try {
      ...
      return {
        analyzeJD: { success: false, result: { coreRequirements: [], keySkills: [] }, error: e.message },
        matchResume: { success: false, result: { matchedSkills: [], matchRatio: 0 }, error: e.message },
      };
    }
  },
...
只返回 JSON。`;
    ...
    try {
      ...
      return {
        draft: { success: false, result: '', error: e.message },
        review: { success: false, result: { issues: [], score: 0 }, error: e.message },
      };
    }
  },
